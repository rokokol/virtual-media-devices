...
| `camera.users` | `[ ]` | Только NixOS. Кого добавить в группу `video` — для сессий, до которых `uaccess` не достаёт. Имя, которого нет в `users.users`, даёт варнинг, а не ошибку: обычно это опечатка, но так же выглядит и юзер, заведённый вне NixOS |
...
```sh
tests/run.sh              # 20 проверок, без модуля ядра и без звукового сервера
```

ffmpeg, pactl, v4l2-ctl и file подставные, так что проверяется командная строка, которую собирает каждый скрипт, — она и есть продукт. Четыре камерных кейса сверяются с эталонами побайтово: цепочка фильтров кончается на `setpts=N/(fps*TB)`, а это единственное, что удерживает картинку от рывка на стыке петли, и меняться случайно она не должна. Устройство — обычный файл во временной папке, `TMPDIR` наведён туда же, так что FIFO микрофона физически не может оказаться в `/tmp`

`nix flake check` гоняет этот набор плюс: упакованные обёртки достают свои инструменты с голого `PATH`, обе настройки доезжают до обёртки (а невыставленная — **не** запекается), оба модуля вычисляются против заглушек опций — включая то, что микрофонная половина не трогает ядро и что всё выключается обратно

## Структура

```
virtual-cam.sh       камера
virtual-mic.sh       микрофон
nix/                 package-cam.nix, package-mic.nix, nixos-module.nix, home-module.nix, module-test.nix
tests/               run.sh, четыре заглушки и эталонные командные строки
